...
class PKM(nn.Module):
    def __init__(self, dim, heads = 4, num_keys = 128, topk = 32, dim_head = 256, input_dropout = 0.3, query_dropout = 0.3, value_dropout = 0.3):
        super().__init__()
    ...
    def forward(self, x, input_mask = None, **kwargs):
        ...
        out = self.values(value_indices, per_sample_weights=attn.to(torch.float))
